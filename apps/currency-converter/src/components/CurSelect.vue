  ...
  <div class="cur-select">
    ...
    >
      <span
        class="flag"
        :style="{
          backgroundImage: `url(https://lipis.github.io/flag-icon-css/flags/4x3/${active
            .toLowerCase()
            .substr(0, 2)}.svg)`
        }"
      ></span>
      ...
      >
        <path
          d="M5 3.3L7.87.44A.8.8 0 019 1.57L5.52 5.01c-.3.3-.77.3-1.06 0L1.01 1.56A.8.8 0 012.14.43L5 3.3z"
        />
      </svg>
    </div>
    ...
    <div v-show="show" v-if="Object.keys(items).length" class="dropdown">
      <div v-for="(item, key) in items" class="item" :key="key">
        <div class="item-content">
          ...
          <svg :class="['star', { fav: isFav(key) }]" @click="toggleFav(key)">
...
/*  hover effects */
.cur-selected:hover .title,
.cur-selected.show .title {
  color: var(--text-color-black);
}
.cur-selected:hover svg,
.cur-selected.show svg {
  fill: var(--text-color-black);
}

.cur-selected:hover .flag,
.cur-selected.show .flag {
  opacity: 1;
}
